...
function init()
  ...
  elseif storage.active then
    self.state:set(activate)
  else
    self.state:set(deactivate)
  end
...
function attack()
  ...
  while hasTarget() do
    local aimVector = world.distance(world.entityPosition(self.target), self.cameraPos)
    setAngle(vec2.angle(aimVector))
    if timer <= 0 then
      world.spawnProjectile(self.projectileType, self.cameraPos, entity.id(), aimVector, false, self.projectileParameters)
...
function getTarget()
  ...
  for _, qItem in pairs(queried) do
    if not storage.useFov and not world.lineTileCollision(self.cameraPos, world.entityPosition(qItem)) then
      updateQueried(qItem)
      ...
      if not world.lineTileCollision(self.cameraPos, world.entityPosition(qItem)) and sightCloseness <= self.halfFov then
        updateQueried(qItem)
        if self.queriedTimings[qItem] < 0 then
          return qItem
        end
...
function hasTarget()
  return world.entityExists(self.target) and not (world.lineTileCollision(self.cameraPos, world.entityPosition(self.target)) or world.magnitude(self.cameraPos, world.entityPosition(self.target)) > self.outOfSightRadius)
end